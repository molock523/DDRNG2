		...
		create_actors= function(self, params)
			...
			local frame= Def.ActorFrame{
				...
				InitCommand=function(subself) 
                    self.middle= subself 
                    subself:setsize(272+4,19):diffuse(color("#ffffff")):faderight(0.5):diffusealpha(0.25)
                end;
                },
			}
			return frame
...
local value_item_mt= {
	__index= {
		create_actors= function(self, params)
			...
			local frame= Def.ActorFrame{
				InitCommand= function(subself)
                    ...
                    self.value_bool= subself:GetChild("vbool"):horizalign(right)
				end,
			}
			local parts= {{
                    thing= Def.Quad{Name="frame"},
                    commands= params.frame_commands or {}
                                        },{
                    thing= Def.Quad{Name="line"},
                    commands= params.frame_commands or {}
                                        },{                    
					...
					thing= Def.BitmapText{Name= "vbool"},
					commands= params.value_text_commands or {}
								        },{                    
		...
		set_value_image= function(self, path)
			self.value_image:visible(false):Load(path)
			self.value_text:visible(true):settext("None")
			self.using_image= true
			if self.info.type == "bool" then
				if self.info.value then
					self.value_image:setstate(1)
                    self.value_text:settext("On")
				else
					self.value_image:setstate(0)
                    self.value_text:settext("Off")
				end
			elseif self.info.type == "choice" then
				if self.info.value then
					self.value_image:setstate(1):visible(false)
                    self.value_text:settext("On")
				else
