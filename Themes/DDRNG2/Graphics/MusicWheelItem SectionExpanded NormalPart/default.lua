...
SetMessageCommand=function(self,params)
    ...
    Def.Sprite{
        InitCommand=function(self)
            if MusicWheelType() == "MusicWheelGrid" then
                self:Load(THEME:GetPathG("MusicWheelItem","SectionExpanded NormalPart/underlay_grid"))
            elseif MusicWheelType() == "MusicWheelList" then
                self:Load(THEME:GetPathG("MusicWheelItem","SectionExpanded NormalPart/underlay_list"))
            end
        end;
    };    
    Def.Banner {
    ...
    Def.Sprite{
        InitCommand=function(self)
            if MusicWheelType() == "MusicWheelGrid" then
                self:Load(THEME:GetPathG("MusicWheelItem","SectionExpanded NormalPart/overlay_grid"))
            elseif MusicWheelType() == "MusicWheelList" then
                self:Load(THEME:GetPathG("MusicWheelItem","SectionExpanded NormalPart/overlay_list"))
            end
        end;
    }; 
	LoadFont("Common Normal")..{
	InitCommand=cmd(addy,100;maxwidth,200;diffuse,color("#000000"));
	SetMessageCommand=function(self, params)
        ...
        if MusicWheelType() == "MusicWheelGrid" then self:y(100):zoom(1)              
